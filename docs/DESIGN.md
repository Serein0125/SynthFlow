...
- **差异视图已移除**（含 `/api/compare`）。原来用 `S.view = 'code' | 'diff'` + `Ctrl+D` 切换，
  但 `Ctrl+D` 是 Monaco 的「选中下一个相同项」（多光标）快捷键 —— 占了它，
  编辑器里就没法做多选，用户直接反馈"代码无法多个字段选中"。
  而差异视图本身价值有限，所以整个功能一起删掉，`Ctrl+D` 交还给 Monaco。
  「本轮改了哪些文件」仍由文件树上的小圆点 + 编辑器里的新增行高亮体现。
...
## 18. 服务容器重载：为什么必须有 dispose（v3.6）

### 18.1 症状与根因

用户报"切换项目后版本时间线还是旧的"。表面上像前端刷新问题，实际是**服务端留了一个还在说话的旧 Runner**：

```
reloadServices():
    svc = buildServices()      ← 旧 svc 直接被丢弃，但**没有任何人停它**
```

旧的 `Runner` 里握着三样还活着的东西：

1. `specTimer` / `commitTimer` / `settleTimer` —— 打字停顿触发的预演与提交定时器
2. 正在跑的 `#consume()` 异步流（模型还在吐 token）
3. 一个闭包引用着**旧项目的** `Session` 与 `Workspace`

只要其中任何一个继续走，`emit` 就会把旧项目的 `state` / `tree` / `versions` 广播出去。
广播是发给**所有 SSE 客户端**的，所以刚切过去的新界面会被旧数据覆盖 ——
版本时间线、文件树、项目徽标一起中招（用户只注意到了时间线，因为那是他最常看的）。

实测：切项目后 2 秒，徽标回到旧项目、时间线回到旧项目的 4 个版本、文件树里出现旧项目刚生成的文件。

### 18.2 修法

**① 废弃标记放在 Runner 内部，而不是包在 emit 外面。**

```js
const rawEmit = emit ?? (() => {});
this.emit = (name, payload) => {
  if (this.disposed) return;   // 废弃之后一个事件都不发
  rawEmit(name, payload);
};
```

一开始我把这层守卫写在 `server.js` 的 emit 包装里，后来挪进了 Runner。理由是可测性：
放在 Runner 内部，"废弃后不许广播"就是一条**单元级不变量**，冒烟测试可以直接构造
一个永不结束的模型流、dispose、再断言零事件。包在 server 层就只能靠端到端测试去碰。

**② `dispose()` 与 `cancel()` 是两件事。**

`cancel()` 是用户动作（停止生成），要广播 `run:cancelled`，界面要给出反馈。
`dispose()` 是生命周期动作（这个 Runner 死了），**必须完全静默** ——
它执行时服务容器已经被替换，任何广播都只会污染新界面。

**③ 前端的 epoch 守卫。**

即使服务端干净了，客户端仍有一类竞态：`refreshAll()` 是串行四次请求，
如果用户在这串请求飞在半路时又切了项目，回来的就是上一个项目的数据。
所以每次 `applyReset` 递增 `S.epoch`，异步回调回来先比对 epoch，不一致就整个丢弃。

**教训：一个"被替换掉的对象"必须有一个明确的死亡时刻。**
只要它还能产生副作用（广播、写盘、改状态），就必须有人负责让它闭嘴 ——
换引用不等于停止运行。

### 18.3 版本删除的几个取舍

- **不能删基线**：删了就没有"回到最初"的锚点，`activeIndex` 也会失去退化目标
- **删当前版本要先回退**：工作区内容就是那个版本，直接从链上摘掉会让游标指向一个不存在的版本。
  所以 `Runner.deleteVersion` 先 `moveVersion('back', 上一版)`（真的恢复工作区），再删条目
- **id 不重排**：删掉 v2 之后保留 `v0, v1, v3`。重排会让快照 id、`preSnapshotId`、
  历史记录里的版本引用全部错位，代价远大于"编号连续"这点美观
- **快照一起删**：快照是全量拷贝，留着就是纯占磁盘。删之前确认没有别的版本引用同一个快照
  （`versions.some(v => v.snapshotId === snapId)`）
- **不在生成中删**：`this.busy` 时直接拒绝，避免与正在落盘的那一轮抢工作区
